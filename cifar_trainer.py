...
class Trainer(nn.Module):
    ...
    def __init__(self, config, args):
        ...
        self.dis = cifar_model.Discriminative(config).cuda()
        self.gen = cifar_model.Generator(image_size=config.image_size, noise_size=config.noise_size).cuda()
        self.enc = cifar_model.Encoder(config.image_size, noise_size=config.noise_size, output_params=True).cuda()
        self.smp = cifar_model.Sampler(noise_size=config.noise_size).cuda()
        ...
        self.labeled_loader, self.unlabeled_loader, self.unlabeled_loader2, self.dev_loader = data.get_cifar_loaders(config)
    ...
    def _train(self, labeled=None, vis=False):
        ...
        g_loss += config.trd_lambda * config.num_label * vi_loss
